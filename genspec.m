function S = genspec(sysf,options) 
%GENSPEC  Generation of achievable fault detection specifications.
%  S = GENSPEC(SYSF,OPTION) determines, in the rows of the binary (logical)
%  array S, all achievable fault detection specifications for the
...
%  OPTIONS.FDFreq    - real frequency value to assess strong specifications 
%                      (Default: [])
...
%  Author:      A. Varga, 25-11-2015.
%  Revision(s): A. Varga, 02-12-2016, 31-07-2017.
%
%  Method: The Procedure GENSPEC from [1] is implemented. The nullspace 
%  method [2] is recursively employed to generate the complete set of 
%  achievable specifications. The method is also described in [3].
...
persistent level tol sdeg smarg FDTol FDGainTol ksave Ts FDFreq
...
if level == 0
   ...
   if isfield(options,'FDFreq')
      ...
      if ~isempty(FDFreq)
         validateattributes(FDFreq, {'double'},{'real','scalar','>=',0},'','OPTIONS.FDFreq')
      end
   else
      FDFreq = [];
   end
